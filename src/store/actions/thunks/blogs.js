...
export const getMyBalance = (amount) => async (dispatch) => {

  dispatch(actions.getBalance.request(Canceler.cancel));

  try {
    const  data  = amount ;

    dispatch(actions.getBalance.success(data)) ;
  } catch (err) {
    dispatch(actions.getBalance.failure(err)) ;
  }
};
