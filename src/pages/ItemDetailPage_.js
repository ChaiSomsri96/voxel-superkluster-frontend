...
import {getListAction , getApprove,isApproved, getAcceptAction ,getBuyAction,getBuyAction_buyer_auction, signMessage, transterItem } from '../core/nft/interact';
...
const ItemDetailRedux = ({ nftId ,colormodesettle }) => {
  ...
  const [AuctionHours , setAuctionHours] = useState();
  const [AuctionMins , setAuctionMins] = useState();
  const [AuctionSecs , setAuctionSecs] = useState();
  ...
  // const isBalanceDispatch= useSelector(selectors.myBalanceState).data;
  ...
  useEffect(()=>{
    // const approve_account = localStorage.getItem('approvedToken'+account) ;
    // console.log(approve_account,'approve_account');
    // setShowBtnState(approve_account);
  ...
  useEffect(()=>{
    let approvedToken_account ;
    // const approvedToken = false ;
    setShowBtnState(localStorage.getItem('approvedToken'+account) == 'true');
    getApproveInfo(account) ;
    // if (approvedToken_account == 'true') {
    //   setShowBtnState(true) ;
    // } else {
    //   setShowBtnState(false) ;
    // }
  ...
  useEffect(() => {
    ...
    if(priceHistoryData && priceHistoryData.length > 0) {
      const tmp = {
        ...
        tooltip: {
          // fields: ['sale_num', 'avg_sale_price'],
          // formatter: (datum) => {
          //   return { name: 'sale num', value: datum.sale_num };
          // },
          customContent: (title, data) => {
            if(!data) return `<div></div>`;
            if(data.length == 0) return `<div></div>`;
            else {
              // return `<div>${data[0].data}</div>`;
  ...
  const lowerPriceItem = async (e, list_id) => {
    e.preventDefault();
    for(let i = 0 ; isListingsData && i < isListingsData.length; i ++) {
      if(isListingsData[i].list_id == list_id) {
        setLowerPriceList(i);
        setLowerPriceQuantity(isListingsData[i].quantity);
      }
    }
  }
  ...
  const cancelListingItem = async (e, list_id) => {
    ...
    await Axios.post(`/api/supply-assets/cancel-list/`, {id:list_id} , { headers: header })
      .then(async (res) => {
        ...
        // const listingsItem = listingsData.data.data;
        // const listingsUsdPrice = listingsData.data.usdPrice;
        ...
        //   return res;
        // }
        // listingsItem.forEach((row, index) => {
        //   if (row.from.public_address == account) {
        //     row = addValueInObject(row, "statusListing", "is_owned");
        //   } else {
        //     row = addValueInObject(row, "statusListing", "not_owned");
        //   }
        //   listingsItem[index] = row;
        // });
        // if (listingsItem) {
        //   setListingsData(listingsItem);
        //   setListingsUsdPrice(listingsUsdPrice);
        // }
        Swal.fire({
          title: 'It worked!',
  ...
  const ActionView = ({content}) => {
    if (accessToken) {
      if (content.from.public_address.toLowerCase() == account?.toLowerCase()) {
        return (
          <div>
            {/* <button className="ant-btn ant-btn-default" onClick={(e) => lowerPriceItem(e, content.list_id)} style = {{marginRight: '10px'}} data-bs-toggle="modal" data-bs-target="#lowerPriceLower_1155">Lower price</button> */}
        ...
        // <button className="ant-btn ant-btn-default" onClick={(e) => buyListingItem(e, content.list_id)}>Buy</button>
  ...
  const calculateTimeLeft = () =>{
    const diff = itemData.auction_end_date != null ? zonedTimeToUtc(new Date(itemData.auction_end_date * 1000) , Intl.DateTimeFormat().resolvedOptions().timeZone)  - new Date() : 0 ;
    if (diff > 0) {
      let hours = Math.floor(diff / (1000 * 60 * 60)) ;
      let minutes = Math.floor((diff / 1000 / 60) % 60) ;
      let seconds = Math.floor((diff / 1000) % 60) ;
      setAuctionHours(hours) ;
      setAuctionMins(minutes) ;
      setAuctionSecs(seconds) ;
    }
  }
  ...
  useEffect(() => {
    // if(remainAuctionEndTime > 0) setTimeout(()=> setRemainAuctionEndTime((remainAuctionEndTime - 1000) >= 0 ? (remainAuctionEndTime - 1000) : 0) ,1000) ;
    // else call_endAuction() ;
    if(isAuctionRemainFlg && remainAuctionEndTime == 0 ) {
      setAuctionProcess(true) ;
      call_endAuction(0) ; 
    }
    // if(isAuctionRemainFlg && remainAuctionEndTime == 0 ) setAuctionProcess(false);
  ...
  useEffect(() => {
    ...
    for(let i = 0; i < isListingsData.length; i ++) {
      ...
      if(_lowPrice < 0) {
        ...
        // listId = isListingsData[i].list_id;
        listId = i;
  ...
  const getItemDetailData = async () => {
    ...
    if (itemDetail) {
      ...
      // setAuctionProcess(itemDetail.auction_end_process) ;
      ...
      const usd_price = itemDetail.price * itemDetail.usdPrice ;
      setUSDPrice(itemDetail.usdPrice) ;
      setETHPrice(itemDetail.ethUsdPrice);
      // localStorage.setItem('usdprice' ,itemDetail.usdPrice) ;
      // setSrcType(itemDetail.type);
      ...
      if (itemDetail.sale_type == 2 && is_721) {
        ...
        const auctionEndUnixTime = new Date(itemDetail.auction_end_date * 1000);
        
        // const TimeZone_now = 'Asia/Kamchatka' ;
        // const today_now = new Date() ;
        // const today_bid = new Date(itemDetail.auction_end_date * 1000) ;
  ...
  const handleApproveAction_between = async () => {
    // 115792089237316195423570985008687907853269984665640564039401584007913129639935
    await getApprove(account, library, itemData)
      .then((res) => {
        if(res == true) {
          localStorage.setItem('approvedToken'+account, true);
        }
  ...
  const handleTransfer = async () => {
    ...
    if (isValidTransferAddr) {
      ...
      try {
        ...
        await transterItem(library, account, transferAddr, tokenId, _transferAmount, collectionAddr, itemData.collection.chain_id, itemData.is_721)
        .then(async(txHash) => {
          Swal.fire({
            title: 'It worked!',
  ...
  const setValidFixedPriceVXL = (value) => {
    // if (isAmountVXL_show * itemData.usdPrice > 10000000) {
    //   value = isAmountVXL_show;
    // }
  ...
  const handleChangeBidPrice = async (e) => {
    const value = e.target.value;
    if (value >= 0) {
      setBidPrice(value)
    } else {
  ...
  const getBidHistoryData = async () => {
    // let current_balance = parseFloat(localStorage.getItem("balance")) - 100000 ;
    // localStorage.setItem("balance",parseFloat(localStorage.getItem("balance")) - 100000) ;
    // dispatch(actions.getMyBalance(current_balance));
    const param = {
      id: nftId
    }
  ...
  const StepC = <CollapsePanelHeader>
          ...
          {/* Confirm <span style={{ color: '#f70dff' }}> VXL</span> Accepting */}
  ...
  // const StepCText = <span>Accept the <span style={{ color: '#f70dff' }}>bidding request</span> in your wallet and wait for your accepting to process.</span>;
  ...
  const listAction = async (chainType,bid_data,idx) => {
    ...
    Axios.post(`/api/sale/accept-item/`, {id:bid_data.id, usdPrice: itemData.usdPrice} , { headers: header })
      .then( async (res) => {
        if(res.data.can_accept == false){
          ...
          modal_acceptBidClose.current.click();
          // console.log('res.data.can_accept',res.data.can_accept) ;
        }else {
          setBidID(idx) ;
          let approve_flg = localStorage.getItem (itemData.collection.name) ;
          let signMsgData;
          ...
          await getListAction(params, signMsgData, rowData ,itemData, library)
            .then(async(res) => {
                ...
                // acceptBidClose.current.click();
                // localStorage.setItem(isItemDetailData.collection.name , true) ;// add for approve
            })
            .catch((err) => {
                modal_acceptBidClose.current.click();
                if(err.code != 4001) {
    ...
    const handleCart = async () => {
      // const postData = {id: itemData? itemData.id}
      const postData = {
        id: itemData.id
      };
      if(isCart) {
    ...
    useEffect(() => {
      ...
      let tmpArr = [];
      for(let i = 0; i < isListingsData.length; i ++) {
        // if(isListingsData[i].from.public_address != account && isListingsData[i].reserve_address != null && isListingsData[i].reserve_address != account) continue;
        tmp ++;
                      ...
                      <TopSubDiv>
                        {
                          ...
                          : <>
                              ...
                              {
                                (itemData.auction_end_process == true && itemData.sale_type == 2 || !is_721 || /*itemData.auction_start_date * 1000 > new Date().getTime()*/ isBidHistoryData.length > 0 && itemData.sale_type == 2) ?
                                  <></>
                                :
              ...
              <div className={(itemData && itemData.warning_message && itemData.warning_message != '') ? "row pt-md-4 Non-scroll-tab" : 'row mt-md-5 pt-md-4 Non-scroll-tab'} style={{overflow:'scroll', maxHeight:'86vh'}}>
                <div className="col-md-6 text-center" style={itemData.asset_type === "3D" ?{maxHeight:'80%',paddingRight:'20px'}:{maxHeight:'80%',paddingRight:'40px'}}  >
                  ...
                  <div style={{ position: 'relative', border:!colormodesettle.ColorMode? '1px solid rgb(53, 56, 64)' : '1px solid rgb(227, 227, 227)', borderBottomRightRadius:'8px', borderBottomLeftRadius:'8px'}}>
                      ...
                      <span style={{width:'100%'}}>
                        ...
                        (
                          itemData.asset_type === "video/mp4" || itemData.asset_type === "audio/mpeg" ?
                           ...
                           : 
                          //  <StyledButton data-bs-toggle="modal" data-bs-target="#auction_buyModal" size="large"  onClick={()=>image_original_show(itemData.image)}>
                              <LazyLoadImage
                               effect="opacity"
                               src={/*itemData.image_preview&&itemData.asset_type !="image/gif" ? itemData.image_preview :*/ (itemData.image? itemData.image : itemData.raw_image?itemData.raw_image:itemData._blob_raw_image?itemData._blob_raw_image:defaultNFT)}
                    ...
                    <div className="moveActionContainer" style={/*moveActionContainer*/{display:'none'}}>
                                            ...
                                            elem.display_type == "number" ?
                                              <div key={i} style={{margin:'5px 17px',width:'auto'}}>
                                                <div className="nft_attr" style={{display:'flex'}}>
                                                  {/* <div className="" style={{display:'flex' , paddingRight:'10px' ,alignItems:'center'}}></div> */}
                                                  <div className="">
                                    ...
                                    <>
                                      <div className="" style={{marginTop:'20px'}}>
                                        ...
                                        <div style={{display:'flex'}}>
                                          ...
                                          <div className="" style={{display:'flex' , justifyContent:'space-between' ,margin:'10px', cursor:'pointer', alignItems: 'center'}}>
                                            {/* <div className="" onClick={()=>authorCollection(itemData.owner.public_address)}> */}
                                            <Tooltip placement="bottom"  title={copyToClipboardTxt}>
                                            <div className="author_list_info" style={voxelNameFont}>
                                              <CopyToClipboard text={itemData.token_id} onCopy={() => setCopyToClipboardTxt('Copied!')}>
                ...
                <div className="col-md-6 Non-scroll-tab" style={{height: '0%', position:width >= 768 ? 'relative':''}}>
                  {/* <div>
                        {
                          getReturnValues_hours(remainAuctionEndTime) + getReturnValues_minutes(remainAuctionEndTime) + getReturnValues_seconds(remainAuctionEndTime) == 0 && isAuctionProcess == true ?
                          <>
                            <br/>
                            {isAuctionProcess}
                            Finalizing auction process....<Spin indicator={antIcon}  />
                            <br/>
                          </>
                          : <></>
                        }
                      ...
                      <div className="col-12 col-md-12 col-lg-12 col-xl-6">
                        {
                          ...
                          <div className="item_info_counts">
                            ...
                            {/* <Tooltip placement="bottom" title="Owned Supply">
                              <div style={{ cursor: 'pointer' }}>
                                <i className="fa fa-user"></i>
                                <span style={{ marginLeft: 5, fontSize: 14 }}>{ownedSupplyNum} </span>
                              </div>
                            </Tooltip> */}
...
{/*  */}
                      ...
                      <div className="mr40">
                        ...
                        <div className="item_author" style={{position:'relative'}} onClick={() => moveToCollectionPage(itemData.collection.link)}>
                          ...
                          <div className="author_list_info" style={{fontWeight:'bold', overflow:'hidden'}}>
                            {/* <span>{display_sttic(itemData.collection.name)}</span> */}
                    ...
                    {/* <div className="spacer-40"></div>
                    <div style={{border:!colormodesettle.ColorMode? 'solid 1px rgb(53, 56, 64)' : 'solid 1px #e3e3e3', width:'100%', borderRadius:'10px', background:!colormodesettle.ColorMode? 'rgb(42, 49, 54)':'white'}}>
                              ...
                              isBidHistoryData.map((data, index) => (
                                <div className="mr40 mb-2" key={index} style = {{overflow:'auto'}}>
                                  <RowDiv className="item_author" style={{display:'flex' , justifyContent:'space-between', whiteSpace:'nowrap', marginBottom:'10px'}}>
                                    ...
                                    <div style ={{}}>
                                      {/* <AcceptButton onClick={()=>setBidID(index)} data-bs-toggle="modal" data-bs-target="#acceptBidModal" >{   isOwnerBtnShow == false ? "" : "Accept" }</AcceptButton> */}
                                      {/* {<StyledButton size="large" diabbled={data.disable_accept == true} onClick={itemData.status == "active" ? () => listAction("onChain" , data,index) : () => listAction("offChain",data,index)} data-bs-toggle="modal" data-bs-target="#listing" >Accept</StyledButton>} */}
                                    {
                                    ...
                                    {
                                          // <StyledButton data-bs-toggle="modal" data-bs-target="#auction_buyModal" size="large" disabled={data.disable_accept} onClick={() => handleBuyAction_auctionBetween(data,index)} >Buy</StyledButton>
                            ...
                            <div style={{padding:'20px 20px', borderTop:!colormodesettle.ColorMode? 'solid 1px rgb(53, 56, 64)' : 'solid 1px #e3e3e3'}}>
                              {
                                isShowMoreStatus ? 
                                  <div >
                                        ...
                                        {/* {consoe.log(isCollectionData ,'iscollectiondata')} */}
        ...
        {/* {console.log(isBidHistoryData[0] ,isBidHistoryData[0]['price'])} */}
              ...
              <div className="modal-content" style={{ borderColor: '#dee2e6', borderRadius: 15 }}>
                  ...
                  <div className="modal-body">
                              ...
                              isBidHistoryData.length > 0 ?
                                <>
                                  <div style={{ textAlign: 'right' ,display:'flex' , justifyContent:'flex-end' , alignItems:'center'}}>
                                      ...
                                      {/* <StyledStatistic className="vxlTotalPrice" value={isUSDPrice && isUSDPrice } prefix={<span style={{ width: 16, height: 16, marginBottom: 5 }}>$</span>} precision={2} /> */}
                                    </div>
                                    <div style={{ textAlign: 'right' }}>
                    ...
                    {/* <ModalBottomDiv>
                      {
                        isShowBtnState == true ?
                          <ModalBtn onClick={handleBuyAction_auctionBetween} disabled={isAgreeWithTerms ? false : true}>Confirm checkout</ModalBtn>
                        : <ModalBtn onClick={handleApproveAction}>Approve</ModalBtn>
                      }
                    </ModalBottomDiv> */}
                  </div>
              </div>
              ...
              <div className="modal-content" style={{ borderColor: '#dee2e6', borderRadius: 15 }}>
                  ...
                  <div className="modal-body">
                    ...
                    <div className="modal-bottom-bar">
                      {
                        isShowBtnState == true ?
                        // <ModalBtn onClick={() => handleBuyAction_auctionBetween(isBidHistoryData[0], 0)} disabled={(isAgreeWithTerms && !isClickConfirm) ? false : true}>Complete auction</ModalBtn>
                  ...
                  <div className="modal-body">
                    <Spin spinning={loadingState} indicator={antIcon} delay={500}>
                      <div style={{ padding: '10px 0px 5px 0px'}}>
                       {
                        ...
                        {/* <DatePicker showTime={{ format: 'HH:mm' }} onChange={onChangeExpiration}  /> */}
                      </div>
                      ...
                      {/* <h5 style={{textAlign:'center' , fontSize:'13px'}}>{
                          saleType === '1' ? 
                            (<>
                            Bid Expiration is {isSaleEndDate}
                            </>)
                          : 
                            (<>
                            Offer Expiration is {isAuctionEndDate}
                            </>)
                        }</h5> */}
                    <ModalBottomDiv>
                      <ModalCancelBtn style={{marginBottom:'5px'}} data-bs-dismiss="modal">Never mind</ModalCancelBtn>
                      {/* {console.log(localStorage.getItem('approvedToken'+account) , localStorage.getItem('approvedToken'+account)==true,'isShowBtnState')} */}
                      {
                        isShowBtnState == true ?
                  ...
                  <div className="modal-body">
                      <StyledCollapse defaultActiveKey={['1', '2']} onChange={callback} expandIconPosition="start">
                          {/* <Panel header={StepA} key="1">
                              ...
                              <Spin spinning={isVerifyAState} indicator={antIcon} delay={500}>
                                {StepAText}
                              </Spin>
                          </Panel> */}
