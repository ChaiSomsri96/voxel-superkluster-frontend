...
const LeftFilterBar = ({ colormodesettle, detail, onFilter }) => {
    ...
    /*
    useEffect(() => {
        onFilter({sale_type: statusData})
    }, [statusData]);

    useEffect(() => {
        onFilter({category: categoryData})
    }, [categoryData]);

    useEffect(() => {
        onFilter({chain: chainData})
    }, [chainData]);
    ...
    return (
        <>
            <FilterSection className="left-filter-section">
