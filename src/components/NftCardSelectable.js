...
const NftCardSelectable = ({
  ...
  const [checkColor, setCheckColor] = useState('pink');
  const [currentColor, setCurrentColor] = useState('pink');

  useEffect(() => {
    let selNFT = JSON.parse(localStorage.getItem('selectedItem'));
    if(!selNFT) return;
    for (let i = 0; i < selNFT.length; i ++) {
      if(nft.id == selNFT[i][0]) {
        setChecked(true);
        setCheckColor(selNFT[i][5]);
      }
    }
  }, [])

  useEffect(() => {
    setCurrentColor(currentCol);
  }, [currentCol])
  ...
  const handleSelect = (e) => {
    // e.preventDefault();
    if(checked == false) setCheckColor(currentColor);
    setChecked(!checked);
    let collection = nft.contract_address;
    if(!nft.is_voxel) collection = nft.collection.contract_address;
    let is_1155 = false;
    if(nft.is_voxel && nft.is_1155) is_1155 = true;
    if(!nft.is_voxel && nft.collection.is_1155) is_1155 = true;
    onSelect(nft.id, nft.token_id, collection, is_1155, nft.is_voxel);
  }
  ...
  useEffect(() => {
    if(checked) {
      const _object = <label className = {checkColor + 'Select'} style ={{position: 'absolute', right: '20px', top: '10px'}}>
                        <input type='checkbox' onChange={(e) => handleSelect(e)} checked = {checked} />
                        <span className = {checkColor + 'CheckBox'}></span>
                      </label>;
      setCheckObject(_object);
      return;
    }

    const _object = <label className = {currentColor + 'Select'} style ={{position: 'absolute', right: '20px', top: '10px'}}>
                      <input type='checkbox' onChange={(e) => handleSelect(e)} checked = {checked} />
                      <span className = {currentColor + 'CheckBox'}></span>
                    </label>;
    setCheckObject(_object);
        ...
        <NFTCard className={className} style={{minWidth:'200px', flex:'1 1 auto', maxWidth:'350px'}} >
          {loadingState && <SkeletonCard />}
          {
            ...
            <div className="nft__item m-0 nftcardpage" style={{ position: 'relative' ,paddingRight:'4px',paddingLeft:'4px',paddingTop:'0px', height:'100%' }}>
              {(nft.supply_number == 1 && nft.status != 'pending') && checkObject}
              {/* { 
              ...
              <a className="author_list_pp hoverauthor" style={{ width: 40, height: 40,marginTop:'6px' ,marginLeft:'6px'}} href = {`collection-detail/${nft.collection.id}`}>
                  <span>
                ...
                <div className="nft__item_info nft_card_sub" style={(get_remain_date_newTemplate(AuctionEndDate) != '0' && nft.auction_start_date * 1000 <= new Date().getTime()) || (get_remain_date_newTemplate(AuctionStartDate) != '0' && nft.auction_start_date * 1000 > new Date().getTime()) ? {paddingRight:'16px',paddingLeft:'16px'}:{paddingTop:'0px',paddingRight:'16px',paddingLeft:'16px'}}>
                  <span>
                    <h4 style={{marginTop:'5px', marginBottom: '20px', whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis'}} >{nft.name}</h4>
                  </span>
                  ...
                  </CardDescription> */}
                  {
                    ...
                    <CardBottom >
                      ...
                      <div className="nItemSale">
                        ...
                        {nft && nft.top_bid && nft.top_bid != "null" ? <><div className="usd_color"><img style={{ width: 13, height: 16, marginBottom: 5 }} src={ethIcon} /> {`${usd_price_set_eth(nft.top_bid)} ($${usd_price_set_usd(nft.to_bid)})`} </div></> : <></>}                          
                    ...
                    <CardBottom >
                      ...
                      <div   style={{ width: 'max-content' }}>
                          ...
                          <div className="nItemSale bid-value mobile-margin" >
                            ...
                            {nft && nft.top_bid && nft.top_bid != "null" ? <><div className="usd_color"><img style={{ width: 16, height: 16, marginBottom: 5 }} src={ethIcon} /> {`${usd_price_set_eth(nft.top_bid)} ($${usd_price_set_usd(nft.to_bid)})`} </div></> : <><div className="usd_color"><img style={{ width: 16, height: 16, marginBottom: 5 }} src={ethIcon} /> {`${usd_price_set_eth(nft.price)} ($${usd_price_set_usd(nft.price)})`} </div></>}
                          </div>
                      </div>
                    </CardBottom>  
                    :
                    ...
                    <CardBottom >
                      <div className="nItemSale" style={{ width: 'max-content' }}>
                          ...
                          <div className="mobile-margin">
                            ...
                            {nft && nft.price && nft.price != "null"  && nft.price > 0  ? <><div className="usd_color"><img style={{ width: 16, height: 16, marginBottom: 5 }} src={ethIcon} /> {`${usd_price_set_eth(nft.price)} ($${usd_price_set_usd(nft.price)})`} </div ></> :<></>}
                          </div>
...
export default memo(NftCardSelectable);
